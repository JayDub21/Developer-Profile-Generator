  ...
  .then(function (userInput) {
    ...
    ghquery(queryUrl).then(function (data) {
      ghQueryStars(queryStarUrl).then(function (responseStars) {
        ...
        pdf
          .create(getHTML(userInput, responseStars, data), options)
          .toFile(`./${userInput.username}.pdf`, function (err, res) {
...
function ghQueryStars(queryStarUrl) {
  return axios.get(queryStarUrl).then(function (response) {
    console.log('Starred Projects: ', response.data.length);

    let responseStars = {
      stars: response.data.length,
    };

    return responseStars;
  });
}
